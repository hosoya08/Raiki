...
function Thunder(options) {
    ...
    this.update = function(index, array) {
        this.segments.forEach(s => { (s.direct += s.change) && Math.random() > 1 && (s.change *= -1) });
...
function loop() {
    update();
    render();
      // 文字がすべて表示されたら終了（繰り返さない）
      const isTextCompleted = text.index >= text.bound.width && thunder.length === 0 && particles.length === 0;
