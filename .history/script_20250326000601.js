...
function Thunder(options) {
  ...
  this.color = options.color || "#ffff33" //雷の色
  ...
  this.update = function (index, array) {
    this.segments.forEach((s) => {
      ;(s.direct += s.change) && Math.random() > 1 && (s.change *= -1)
    })
...
function Text(options) {
  ...
  this.color = options.color || "#e60012" //文字の色
  this.delay = options.delay || 2
  this.basedelay = this.delay
  buffer.font = `${this.size}px cofo-kabeltouw-solid-0`
...
function loop() {
  update()
  render()
 // すべての文字が表示され、雷と火花も消滅したらループ終了
 const textFinished = text.index >= text.bound.width;
 const animationFinished = thunder.length === 0 && particles.length === 0;

 if (!textFinished || !animationFinished) {
...
function update() {
  text.update()
  thunder.forEach((l, i) => l.update(i, thunder)) // 雷の演出（繰り返し処理
  particles.forEach((p) => p.update()) //火花の演出（繰り返し処理
}
